...
static bool preproc_params(Relation const *node, Slice const slice, OJob *job) {
        ...
        OJob_1_or_2LF(job, slice); // todo version with line continuation
        return true;
}

static bool preproc_arg(Relation const *node, Slice const slice, OJob *job) {
        if (is_first_child(node)) {
                return true;
        }
        if (me(node) == sym_comment | unknown(node)) {
...
static bool preproc_ifdef(Relation const *node, Slice const slice, OJob *job) {
        ...
        if (sym(Nodes_at(node->nodes, 1)) == sym_identifier 
        & sym(Nodes_at(node->nodes, 2)) == aux_sym_preproc_def_token1
        & sym(Nodes_at(node->nodes, 3)) == sym_preproc_def
        & sym(Nodes_at(node->nodes, 4)) == sym_identifier
        & sym(Nodes_at(node->nodes, 5)) == aux_sym_preproc_ifdef_token2
        & sym(Nodes_at(node->nodes, 6)) == sym_preproc_ifdef) {
                /* #ifndef FOO_H   */
                /* #define FOO_H\n\n */
                /*              ^ ^  */
                OJob_2LF(job);
...
static bool translation_unit(Relation const *node, Slice const slice, OJob *job) {
        ...
        // if (sym(child(node, node->child_idx - 1)) == sym_function_definition) {
        //         OJob_2LF(job);
        //         return true;
        // }
        if (me(node) == anon_sym_SEMI) {
...
static bool declaration_list(Relation const *node, Slice const slice, OJob *job) {
        ...
        if (sym(Nodes_at(node->nodes, 1)) == sym_preproc_directive
        & sym(Nodes_at(node->nodes, 2)) == sym_preproc_call
        & sym(Nodes_at(node->nodes, 3)) == anon_sym_LBRACE
        & sym(Nodes_at(node->nodes, 4)) == sym_declaration_list) {
                /* #ifdef __cplusplus  */
                /* extern "C" {        */
                /* #endif\n\n          */
                /*         ^           */
...
static bool for_statement(Relation const *node, Slice const slice, OJob *job) {
        ...
        | me(node) == anon_sym_RPAREN) {
                if (single_line_PAREN) {
                        if (sym(Nodes_at(node->nodes, 1)) == anon_sym_LPAREN) {
                                /* (int i = 0; i < max; i++) */
                                /* ^^                        */
                                return true;
                        }
...
static bool case_statement(Relation const *node, Slice const slice, OJob *job) {
        ...
        if (me(node) == sym_compound_statement 
        && sym(Nodes_at(node->nodes, 1)) == anon_sym_COLON
        && is_last_child(node)) {
                /* case ABC: { .... } case */
                /*          ^              */
                OJob_space(job);
